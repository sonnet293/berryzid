...
let thumbnailBlob    = null;   // 썸네일 업로드용 Blob
...
onAuthStateChanged(auth, (user) => {
  ...
  loginSection.style.display  = isOwner ? "none" : "flex";
...
loginBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (user.uid !== OWNER_UID) {
      await signOut(auth);
      loginError.textContent = "접근 권한이 없습니다.";
    }
  } catch {
...
// ─────────────────────────────────────────────
const PRESETS = [
  "#000000","#ffffff","#f03e3e","#e67700","#f59f00",
  "#37b24d","#1c7ed6","#7048e8","#d6336c","#495057",
  "#868e96","#ced4da","#ff6b6b","#ffa94d","#ffd43b",
  "#69db7c","#4dabf7","#9775fa","#f783ac","#dee2e6",
];
...
function closeColorPanel() { colorPanel.classList.remove("active"); }
...
function syncFromRGB() {
  ...
  numR.value = currentR; numG.value = currentG; numB.value = currentB;
...
// 폰트 추가
// ─────────────────────────────────────────────
customFonts.forEach(applyCustomFont);

/**
 * 폰트 데이터 구조
 * - type: "google" | "webfont"
 * - label: 표시 이름 (예: "나눔손글씨 행복체")
 * - cssName: Quill용 클래스 키 (영소문자+하이픈, 예: "nanum-haengbok")
 * - url: Google Fonts일 때 @import URL
 * - fontFaceCSS: webfont일 때 @font-face 코드 전체
 */
function applyCustomFont({ label, cssName, url, fontFaceCSS, type }) {
  // 1. 폰트 CSS 주입
  if (type === "webfont") {
    // @font-face 코드를 <style>로 주입
    ...
    if (!document.querySelector(`link[data-font="${cssName}"]`)) {
      ...
      link.rel = "stylesheet"; link.href = url; link.dataset.font = cssName;
      document.head.appendChild(link);
    }
  }

  // 2. Quill font-family 매핑 style 주입
  ...
  // 4. 툴바 select에 option 추가
  const select = document.getElementById("font-select");
  if (select && !select.querySelector(`option[value="${cssName}"]`)) {
    const opt = document.createElement("option");
    opt.value = cssName; opt.textContent = label; select.appendChild(opt);
  }
  const qlSelect = document.querySelector(".ql-font");
  if (qlSelect && !qlSelect.querySelector(`option[value="${cssName}"]`)) {
    const qlOpt = document.createElement("option");
    qlOpt.value = cssName; qlOpt.textContent = label; qlSelect.appendChild(qlOpt);
  }
}
...
// ── 추가 확인 ──
fontModalConfirm.addEventListener("click", () => {
...
function _saveFont(fontData) {
  customFonts = customFonts.filter(f => f.cssName !== fontData.cssName);
...
resetBtn.addEventListener("click", () => {
  if (confirm("작성 중인 내용을 초기화할까요?")) {
    titleInput.value = "";
    quill.setContents([]);
    thumbnailRemove.click();
    secretToggle.checked = false;
    secretToggle.dispatchEvent(new Event("change"));
  }
});
...
submitBtn.addEventListener("click", async () => {
  ...
  const isSecret    = secretToggle.checked;
  const secretPw    = secretPassword.value.trim();

  if (!title)       return showToast("제목을 입력해주세요.", true);
  if (!textContent) return showToast("내용을 입력해주세요.", true);
  if (isSecret && secretPw.length < 4) return showToast("비밀번호는 4자리 이상 입력해주세요.", true);
  ...
  try {
    ...
    let thumbnailUrl = "";
    if (thumbnailBlob) {
      const ext = (thumbnailBlob.name?.split(".").pop() || "jpg").toLowerCase();
      thumbnailUrl = await uploadImage(thumbnailBlob, `thumb.${ext}`);
    }

    // 콘텐츠에서 첫 번째 이미지 자동 추출 (썸네일 없을 때 fallback)
    if (!thumbnailUrl) {
      const doc = new DOMParser().parseFromString(content, "text/html");
      const firstImg = doc.querySelector("img");
      if (firstImg) thumbnailUrl = firstImg.src;
    }
    ...
    await addDoc(collection(db, "posts"), {
      ...
      thumbnailUrl,
      isSecret,
      // 비밀글이면 비밀번호 저장 (평문 저장 — 간단한 블로그용)
      secretPassword: isSecret ? secretPw : "",
      authorUid: currentUser.uid,
      createdAt: serverTimestamp(),
      updatedAt: serverTimestamp(),
    });

    showToast("게시글이 저장됐어요 ✓");
    titleInput.value = "";
    quill.setContents([]);
    thumbnailRemove.click();
    secretToggle.checked = false;
